...
class SpectralMixtureGPModel(gpytorch.models.ExactGP):
    ...
    # TODO: Is this a reasonable init??
    ...
    def covariance_state_string(self):
        log_str = """
########## Covariance Module State #########
Raw Mixture Weights: %s
Raw Mixture Scales: %s
Raw Mixture Means: %s
"""
        cm = self.covar_module.state_dict()
        return log_str % (cm['raw_mixture_weights'],
                          cm['raw_mixture_means'][:, 0, 0],
                          cm['raw_mixture_scales'][:, 0, 0])
...
class GPR_DC:
    ...
    def _all_preds(self, X, **kwargs):
        ...
        preds_scaled = [
            self._ynorm[1][k] * p + self._ynorm[0][k]
...
def _fit_gpr(GPR_TYPE,
             ...
             num_restarts=9):
    logger.info('Fiting GPR (num_iter: %d, num_restarts: %d)' %
                (num_iter, num_restarts))
    min_loss = 0.0
    min_model = None
    for i in range(num_restarts):
        new_model = GPR_TYPE(X, y, likelihood)
        logger.debug(new_model.covariance_state_string())
        loss_i = _run_fit_gpr(new_model, likelihood, X, y, num_iter,
                              debug_log_interval)
