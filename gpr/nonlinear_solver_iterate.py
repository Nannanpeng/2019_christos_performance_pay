...
from .ipopt_wrapper import EV_F_ITER, EV_GRAD_F_ITER, EV_G_ITER, EV_JAC_G_ITER, sparsity_hess, sparsity_jac_g
import numpy as np
import ipyopt
...
def iterate(k_init, n_agents, gp_old):
    ...
    def eval_jac_g(x):
        return EV_JAC_G_ITER(X, k_init, n_agents)

    # First create a handle for the Ipopt problem
    nlp = ipyopt.create(N, X_L, X_U, M, G_L, G_U, sparsity_jac_g(N, M),
